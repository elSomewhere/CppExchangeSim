...
#include <vector>
#include <optional>
// #include <atomic> // Removed: Order book is synchronous
...
public:
    ...
    LOBOrder& operator=(const LOBOrder&) = delete; // Cannot assign due to const uoid_
    LOBOrder& operator=(LOBOrder&&) noexcept = delete; // Cannot assign due to const uoid_
...
public:
    ...
    LOBFillResult& operator=(const LOBFillResult&) = delete; // Explicitly deleted due to const member
    LOBFillResult& operator=(LOBFillResult&&) noexcept = delete; // Explicitly deleted due to const member
...
public:
    ...
    LOBClearResult& operator=(const LOBClearResult&) = delete; // Explicitly deleted due to const member
    LOBClearResult& operator=(LOBClearResult&&) noexcept = delete; // Explicitly deleted due to const member
...
public:
    ...
    // New public method to get the list iterator
...
public:
    ...
    LOBClearResult clear_quantity(SIZE_TYPE& quantity_to_clear, std::vector<ID_TYPE>& exhausted_order_uoids_at_this_level) {
        ...
        if constexpr (i == DOUBLEOPTION::FRONT) {
            ...
            while (it != container.begin() && quantity_to_clear > 0) {
                --it;
                LOBOrder& currentOrder = *it;
                SIZE_TYPE tradeQuantity = std::min(quantity_to_clear, currentOrder.quantity_);
...
public:
    ...
    std::tuple<SIZE_TYPE, std::vector<LOBClearResult>> limit_match_price_quantity(PRICE_TYPE price, SIZE_TYPE quantity) {
        ...
        while (it != counter_book.end() && quantity > 0 && price_val_comparator((*it)->price_, price)) {
            ...
            if (price_node->get_total_quantity() == 0) {
                PRICE_TYPE key_to_erase = price_node->price_; // Get key before it's deleted with unique_ptr
    ...
    std::optional<std::tuple<ID_TYPE, Price*>> book_price_quantity(PRICE_TYPE price, SIZE_TYPE quantity) {
        ...
        auto order_list_it = pricenode_ptr->template insert_order<BookOrderPriority>(new_uoid, quantity);
    ...
    std::tuple<SIZE_TYPE, std::vector<LOBClearResult>> market_match_quantity(SIZE_TYPE quantity) {
        ...
        while (it != counter_book.end() && quantity > 0) {
            ...
            if (price_node->get_total_quantity() == 0) {
                PRICE_TYPE key_to_erase = price_node->price_;
                it = counter_book.erase(it);
            } else {
                ++it;
            }
    ...
    std::optional<ModifyVolResult> modify_limit_order_vol(ID_TYPE order_id, SIZE_TYPE new_volume) {
        ...
        if (new_volume <= 0) {
            ...
            return ModifyVolResult(current_price, old_volume, 0, true, std::nullopt);
        }
        ...
        if constexpr (PriorityOption == TRIPLEOPTION::INPLACE) {
            ...
            return ModifyVolResult(current_price, old_volume, new_volume, false, std::nullopt);
        } else {
            pricenode->remove_order_from_container(order_id);
            uoid_to_price_.erase(order_id);
            ...
            std::list<LOBOrder>::iterator new_order_it;
            if constexpr (PriorityOption == TRIPLEOPTION::FRONT) {
                new_order_it = pricenode->template insert_order<DOUBLEOPTION::FRONT>(new_gen_uoid, new_volume);
            } else {
                new_order_it = pricenode->template insert_order<DOUBLEOPTION::BACK>(new_gen_uoid, new_volume);
            }
            uoid_to_price_[new_gen_uoid] = pricenode;

            return ModifyVolResult(current_price, old_volume, new_volume, false, new_uoid_opt);
        }
    }
    ...
    std::optional<ModifyVolResult> remove_limit_order_vol(ID_TYPE order_id, SIZE_TYPE cancel_amount) {
        ...
        SIZE_TYPE new_volume = order->quantity_ - cancel_amount;
        return modify_limit_order_vol<CompUniquePtr, PriorityOption>(order_id, new_volume < 0 ? 0 : new_volume);
    }
    ...
    std::optional<std::tuple<ID_TYPE, ReplaceOrderResult>> replace_limit_order_vol(ID_TYPE order_id_old, SIZE_TYPE volume_new) {
        ...
        pricenode->total_quantity_ -= old_volume;
        auto next_iter_hint_for_inplace = pricenode->container.erase(old_order_list_iter);
        ...
        std::list<LOBOrder>::iterator new_order_final_iter;
        if constexpr (PriorityOption == TRIPLEOPTION::INPLACE) {
            new_order_final_iter = pricenode->insert_order_at_position(next_iter_hint_for_inplace, order_id_new, volume_new);
        } else if constexpr (PriorityOption == TRIPLEOPTION::FRONT) {
            new_order_final_iter = pricenode->template insert_order<DOUBLEOPTION::FRONT>(order_id_new, volume_new);
        } else {
            new_order_final_iter = pricenode->template insert_order<DOUBLEOPTION::BACK>(order_id_new, volume_new);
        }
    ...
    std::optional<ModifyPriceResult> modify_limit_order_price(PRICE_TYPE new_price, ID_TYPE order_id_old) {
        auto map_it = uoid_to_price_.find(order_id_old);
        if (map_it == uoid_to_price_.end()) {
            // This is a valid case: client trying to modify a non-existent order.
            return std::nullopt;
        }
        ...
        if constexpr (PriorityOption == TRIPLEOPTION::INPLACE) {
            ...
            uoid_to_price_[order_id_old] = new_pricenode_ptr;

            return ModifyPriceResult(old_price, original_volume, order_id_old);

        } else { // TRIPLEOPTION::FRONT or TRIPLEOPTION::BACK (generates new UOID)
            auto removed_data = old_pricenode->remove_order_from_container(order_id_old);
            assert(removed_data.has_value() && removed_data->uoid_ == order_id_old && "Failed to remove order for FRONT/BACK price change.");
            uoid_to_price_.erase(order_id_old);
            ...
            auto book_result = book_price_quantity<CompUniquePtr, bookPriority>(new_price, original_volume);

            assert(book_result.has_value() && "book_price_quantity failed unexpectedly for non-zero volume order.");
            return ModifyPriceResult(old_price, original_volume, std::get<0>(*book_result));
        }
    }
    ...
    std::optional<ModifyPriceVolResult> modify_limit_order_price_vol(PRICE_TYPE new_price, SIZE_TYPE new_volume, ID_TYPE order_id_old) {
        auto map_it = uoid_to_price_.find(order_id_old);
        if (map_it == uoid_to_price_.end()) {
            // Valid case: client trying to modify a non-existent order.
            return std::nullopt;
        }
        ...
        PRICE_TYPE old_price = old_pricenode->price_;
        SIZE_TYPE old_volume = old_order_modifiable_ptr->quantity_; // Known to be > 0

        if (new_volume <= 0) { // Order is being removed (cancelled or reduced to zero). This is a valid operation.
            auto removed_data = old_pricenode->remove_order_from_container(order_id_old);
            assert(removed_data.has_value() && removed_data->uoid_ == order_id_old && "Failed to remove order for zero new volume.");
            uoid_to_price_.erase(order_id_old);
        ...
        // From here, new_volume > 0

        if constexpr (PriorityOption == TRIPLEOPTION::INPLACE) {
            if (old_price == new_price) { // Price is same, only volume changes. UOID and queue position preserved.
            ...
            } else { // Price changes, volume changes (new_volume > 0). UOID is retained.
                auto removed_data = old_pricenode->remove_order_from_container(order_id_old);
                assert(removed_data.has_value() && removed_data->uoid_ == order_id_old && "Failed to remove order for INPLACE price_vol change.");
                ...
                new_pricenode_ptr->template insert_order<DOUBLEOPTION::BACK>(order_id_old, new_volume);
                uoid_to_price_[order_id_old] = new_pricenode_ptr;

                return ModifyPriceVolResult(old_price, old_volume, new_volume, old_level_removed_flag, order_id_old);
            }
        } else { // TRIPLEOPTION::FRONT or TRIPLEOPTION::BACK (generates new UOID since new_volume > 0)
            auto removed_data = old_pricenode->remove_order_from_container(order_id_old);
            assert(removed_data.has_value() && removed_data->uoid_ == order_id_old && "Failed to remove order for FRONT/BACK price_vol change.");
            uoid_to_price_.erase(order_id_old);
            ...
            constexpr DOUBLEOPTION bookPriority = (PriorityOption == TRIPLEOPTION::FRONT) ? DOUBLEOPTION::FRONT : DOUBLEOPTION::BACK;
            auto book_result = book_price_quantity<CompUniquePtr, bookPriority>(new_price, new_volume);

            assert(book_result.has_value() && "book_price_quantity failed unexpectedly for non-zero new_volume.");
            return ModifyPriceVolResult(old_price, old_volume, new_volume, old_level_removed_flag, std::get<0>(*book_result));
        }
    }

    // New method to get the price of an order
    ...
    SIZE_TYPE get_volume_for_price(PRICE_TYPE target_price) const {
        ...
        for (const auto& pricenode_uptr : counter_book) {
            if (price_val_comparator(pricenode_uptr->price_, target_price)) {
                ...
                if constexpr (std::is_same_v<CompUniquePtr, PriceUniquePtrCompareDescending>) {
                    if (pricenode_uptr->price_ > target_price) break;
                } else {
                    if (pricenode_uptr->price_ < target_price) break;
                }
            }
