...
namespace EventBusSystem {
    ...
    public:
        TopicBasedEventBus(Timestamp start_time = Timestamp{}) : current_time_(start_time) {
            ...
            // Initialize latency distribution (example: mean 1000us, sigma 0.5 for lognormal)
            // For lognormal: mu = log(mean_latency) - 0.5 * sigma^2; sigma_dist = sigma
            // Or more simply, directly use mean and sigma as arguments if supported by library for lognormal,
            // but std::lognormal_distribution takes mu and sigma of the underlying normal distribution.
            // Python's np.random.lognormal(mean=np.log(mean_latency_us), sigma=sigma)
            // So, if mean_latency_us = 1000, sigma_param = 0.5
            // mu_for_dist = std::log(1000.0); sigma_for_dist = 0.5;
            // latency_distribution_ = std::lognormal_distribution<double>(mu_for_dist, sigma_for_dist);
            double mean_latency_us = 1000.0;
            double sigma_param = 0.5; // This is the sigma for the lognormal, not the underlying normal.
            // For std::lognormal_distribution, sigma is for the normal.
            // If Python's sigma is for the lognormal, then for std::lognormal:
            // underlying_normal_mu = log(desired_lognormal_mean) - 0.5 * underlying_normal_sigma^2
            // underlying_normal_sigma = sigma_param
            // We need to be careful here. Python's `np.lognormal(mean, sigma)`
            // means `mean` is `mu` of underlying normal, `sigma` is `sigma` of underlying normal.
            // So `std::lognormal_distribution<double>(std::log(mean_latency_us), sigma_param)` should be equivalent.
        ...
        // --- Simulation Step (MODIFIED Logging) ---
        Timestamp step() {
            if (event_queue_.empty()) {
                return current_time_;
            }

            ScheduledEvent current_event = std::move(const_cast<ScheduledEvent&>(event_queue_.top())); // Need to cast away const for move
            event_queue_.pop();

            // Update current time: should only advance.
            if (current_event.scheduled_time < current_time_) {
                ...
                // This indicates a flaw in scheduling logic or priority queue comparison.
                // Forcing current_time_ forward, but this is a bug.
            }
            ...
            class ProcessingGuard { /* ... (unchanged) ... */
            ...
            try {
                ...
                receiver->process_event_variant(
                        ...
                        current_time_, // Pass the updated current_time_
