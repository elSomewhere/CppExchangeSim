...
void EventModelExchangeAdapter::_on_limit_order_acknowledged(
    ...
    publish_wrapper("LimitOrderAckEvent", stream_id_str, ack_event);

    if (xid != ID_DEFAULT && remaining_qty == 0) {
        _remove_order_mapping(xid);
    }
    // If xid != ID_DEFAULT and it rested (even partially), _process_limit_order should have mapped it.
    // If xid was ID_DEFAULT from place_limit_order (full taker fill), but ExchangeServer now reports
    // a new resting xid in this ack (if its logic allows this), we'd need to map it here.
    // Current ExchangeServer::place_limit_order returns the resting_order_id_if_any which is used for mapping.
}
